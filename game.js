...
const isPause = () => state.menus.active === MENU_PAUSE;


const highScoreKey = "__ninja-cutter__highScore";
const getHighScore = () => {
...
const colorToHex = (color) => {
  return (
    "#" + (color.r | 0).toString(16).padStart(2, "0") + (color.g | 0).toString(16).padStart(2, "0") + (color.b | 0).toString(16).padStart(2, "0")
  );
};
...
const shadeColor = (color, lightness) => {
  ...
  return (
    "#" +
    (lerp(color.r, other, mix) | 0).toString(16).padStart(2, "0") + (lerp(color.g, other, mix) | 0).toString(16).padStart(2, "0") + (lerp(color.b, other, mix) | 0).toString(16).padStart(2, "0")
  );
};
...
const makeSpawner = ({ chance, cooldownPerSpawn, maxSpawns }) => {
  ...
  return {
    ...
    mutate(options) {
      if (options.chance) chance = option.chance;
      cooldown.mutate({
        rechargeTime: options.cooldownPerSpawn, units: options.maxSpawns
      });
    }
  };
};
